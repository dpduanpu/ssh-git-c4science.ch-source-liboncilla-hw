...
public:
	...
	void initialize(bool isReversed, bool isHip, int16_t motorPositionLimit,
			int16_t motorCurrentPosition);

	/// The next function should not be public, but friends of the SBCPQueue...
	double nodeToQueueJointPosition();
	void queueToNodeJointPosition(int16_t magneticEncoderVal,
			int16_t magneticEncoderStatus, int16_t motorEncoderVal);
